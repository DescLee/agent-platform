...
rem Stop the Green Giant Tauri development client started from this workspace.
set "ROOT=%~dp0"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if (-not $process) { Write-Host 'Green Giant is not running from this workspace.'; exit 0 };" ^
  "taskkill.exe /PID $process.ProcessId /T /F ^| Out-Null;" ^
  "if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE };" ^
  "Write-Host 'Green Giant stopped.'"
