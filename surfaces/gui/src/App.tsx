      ...
      <div className={"main" + (surface === "session" && agent !== "chat" && !railHidden ? " rail-open" : "")}>
        ...
        <div className={"main-workspace" + (railHidden ? " rail-hidden" : "")}>
          <div className="main-chat">
            ...
            <Composer
              expertSlot={agent !== "cowork" ? <button className="group text-[13px] text-muted hover:text-ink inline-flex items-center rounded-md bg-paper px-2 py-1" onClick={() => pickCoworker()}><span className="mr-1 opacity-0 group-hover:opacity-100">×</span>{fullPersonaName(personas.find((p) => p.id === agent)?.name || agent, agent)}</button> : undefined}
              mode={mode}
              model={model}
              models={models}
